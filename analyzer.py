...
TRAIN_SELECTED_FILE = os.path.join(config.CACHE_DIR, 'train_selected.csv')
TAG_HIST_FILE = os.path.join(config.CACHE_DIR, 'tag_hist.dat')
TAG_COUNT_FILE = os.path.join(config.CACHE_DIR, 'tag_count.dat')
IMPORTANT_TAGS_FILE = os.path.join(config.CACHE_DIR, 'important_tags.dat')
...
def important_tags(hist, k=2000):
    ...
    with open(TRAIN_FILE, 'r') as fh:
        ...
        for row in reader:
            ...
            if any([True for e in tags.split() if e in selected_tags]):
                records.append(row)
    with open(TRAIN_SELECTED_FILE, 'w') as fh:
